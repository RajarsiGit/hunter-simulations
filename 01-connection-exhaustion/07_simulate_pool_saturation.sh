...
echo ""
echo "Clean up early via RDS-side termination (these sessions are 'active', not"
echo "idle-in-transaction, so use the generic cleanup script, not 02):"
echo "  psql -h ${PGHOST} -p 5432 -U <superuser> -d ${PGDATABASE} \\"
echo "    -v app_pattern='%drill_pool_saturation%' \\"
echo "    -f 08_cleanup_drill_sessions.sql"
echo ""
