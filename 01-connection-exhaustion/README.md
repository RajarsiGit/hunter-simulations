...
| `02_terminate_sessions.sql` | Senthil §9.3 | Dry-run-by-default session termination |
...
| `05_throttle_and_drain.sql` | Senthil §9.4/§9.5 | Role/DB throttle templates + PgBouncer emergency commands |
...
| `08_cleanup_drill_sessions.sql` | — | Generic `drill_*` kill-switch |
...
`run_all.sh` runs every drill in this folder back-to-back, then the
detection scripts, then cleanup — one command instead of stepping through
06/07/09/10/11 by hand. See `--list` for a full flag reference.
...
# Doc-example scale (slower, closer to a real incident's shape)
./run_all.sh --full --yes
```
...
# Mitigate (preview, then execute)
psql -v min_idle_minutes=0 -f 02_terminate_sessions.sql
psql -v dry_run=0 -v min_idle_minutes=0 -f 02_terminate_sessions.sql
...
# Force-end any drill early
psql -v app_pattern='%drill_%' -f 08_cleanup_drill_sessions.sql
```

## Notes

- `01`/`02`/`04`/`06`/`08`/`09`/`10` connect **directly to RDS** (port 5432) so `pg_stat_activity` reflects true backend state.
...
- Every `.sh` drill tags sessions with a unique `application_name` (`drill_idle_txn`, `drill_pool_saturation`, `drill_role_limit`, `drill_idle_conn_storm`, `drill_session_pinning`) so `08_cleanup_drill_sessions.sql -v app_pattern='%drill_%'` always finds and removes them.
