...
for i in $(seq 1 "${ATTEMPTS}"); do
    if PGPASSWORD="${DRILL_ROLE_PASSWORD}" psql -h "${PGHOST}" -p "${PGPORT}" -U "${DRILL_ROLE}" -d "${PGDATABASE}" \
         -c "SET application_name='drill_role_limit'; SELECT pg_sleep(30);" >/tmp/drill_attempt_${i}.log 2>&1 &
    then
        pid=$!
        sleep 0.3
...
echo ""
echo "Waiting for any remaining open drill connections to finish (max 30s)..."
wait
