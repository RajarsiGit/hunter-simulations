...
Every hold-type default below is ~6-8s unless noted — sized so each drill
completes in well under 20s for fast local/CI drilling. This is short of the
locks-deadlocks-blocking-queries hunter's 300s poll interval (and, for 02/11,
LB-3's 300s idle-in-txn threshold), so these defaults are NOT reliable for
hunter-detection runs — pass a larger hold value (e.g. 900) per script for
that. Every session explicitly disables the SysCloud baseline
`statement_timeout`/`lock_timeout`/`idle_in_transaction_session_timeout`
(runbook §7.3: 5min/10s/60s) for itself — those settings would otherwise
kill/abort every drill session long before even the short durations below
are reached. The deadlock scripts (03, 07) also `SET deadlock_timeout='2s'`
so deadlock detection itself doesn't eat into the 20s budget (production
baseline is 15s).
...
| `08_simulate_mvw_refresh_lock.sh` | MVW REFRESH (blocking mode, 8s hold) vs. REFRESH CONCURRENTLY; optional duplicate injection | Blocking mode: AccessExclusiveLock on MVW; concurrent + duplicate: unique index violation |
...
| `14_simulate_vacuum_full_blocking.sh` | Mode A: VACUUM FULL holds AccessExclusiveLock; Mode B: long transaction blocks vacuum horizon (8s) | Mode A has a real coverage gap: `lock_test_accounts` is only 5 rows, so VACUUM FULL finishes in milliseconds regardless of hold settings — see script header |
...
confirmation prompt — every drill fires immediately, completes in well under
20s (see table above), and leaves its sessions/tables in place afterward for
inspection.
...
./02_simulate_row_lock_blocking.sh 1 8 --yes
...
./18_simulate_lock_queue_amplification.sh 6 12 --yes
psql -h "$PGHOST" -p "${PGPORT:-5432}" -U "$PGUSER" -d "$PGDATABASE" -f 09_lock_triage_queries.sql
psql -h "$PGHOST" -p "${PGPORT:-5432}" -U "$PGUSER" -d "$PGDATABASE" \
